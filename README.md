# Bumsu Kim · 김범수

**Integrated M.S./Ph.D. Student · Hanyang University**  
[Intelligent Mobile Computing Lab (IMC Lab)](https://imc.hanyang.ac.kr/)

I am interested in **self-improving and self-evolving AI**, especially how multimodal language models can learn from feedback and experience. My research background includes industrial anomaly detection and network intrusion detection.

[Website](https://withsu.github.io/) · [Research Interests](https://withsu.github.io/research/) · [CV](https://withsu.github.io/cv/) · [Email](mailto:kbsbmori3@gmail.com)

## Research Interests

- **MLLM self-improvement & self-evolution** — iterative learning from feedback and experience.
- **Multimodal learning** — learning from complementary information across modalities.
- **Industrial anomaly detection** — detecting anomalous objects and patterns in industrial settings.

## Education & Research Experience

| Period | Institution / Lab | Role & focus |
| :--- | :--- | :--- |
| Sep 2026 – Present | Hanyang University | Integrated M.S./Ph.D. student |
| Jan 2026 – Present | Intelligent Mobile Computing Lab, Hanyang University | Research in self-improvement, multimodal learning, and anomaly detection |
| Sep 2024 – Dec 2025 | CBNU Robotics Lab | Undergraduate researcher · semiconductor microdevice object anomaly detection |

Undergraduate studies: **Information & Communication Engineering, Chungbuk National University**.

## Selected Publications

| Year | Publication | Venue · Authorship |
| :--- | :--- | :--- |
| 2025 | **AdVise: A Real-time Personalized Advertising System Using Gaze Tracking and Deep Reinforcement Learning** | ICCC 2025 · First author |
| 2025 | **Lightweight Hybrid Network Intrusion Detection Method based on AutoEncoder-XGBoost** | ICTC 2025 · Second author |
| 2025 | **AutoEncoder-XGBoost 기반 경량 하이브리드 네트워크 침입 탐지 방법** | 한국통신학회 하계종합학술발표회 · 제1저자 |
| 2025 | **PCB 영상에서 납땜 영역을 포함한 초소형 부품 탐지를 위한 계층적 방법** | ICROS 2025 · 제1저자 |

## Selected Awards

| Year | Award | Research / project |
| :--- | :--- | :--- |
| 2025 | 국제 산업보안 논문 경진대회 · 장려상 | 동적 환경 적응형 제로데이 탐지 시스템 연구 |
| 2025 | 교내 UROP · 우수상 | PCB 영상에서 초소형 부품 탐지를 위한 계층적 방법 |
| 2025 | 재단법인 미래와 소프트웨어 정보보안 공모전 · 장려상 | AI 기반 정보보안 통합 서비스 개발 |


---

More about my research and experience: **[withsu.github.io](https://withsu.github.io/)**.
